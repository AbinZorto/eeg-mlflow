...
A third ablation addressed the concern that the hybrid architecture, at \texttt{inner-k=1}, might exploit batch-level or distributional artifacts rather than the selected feature. After feature selection, all selected feature values were replaced with i.i.d.\ Gaussian noise ($\mathcal{N}(0, 1)$) and the model was retrained under the identical LOPO configuration. Performance fell to ROC-AUC 0.408 (balanced accuracy 0.393, F1 0.250), indistinguishable from chance. This confirms that the model relies on the actual feature values and is not driven by distributional cues accessible through batch normalization statistics.
